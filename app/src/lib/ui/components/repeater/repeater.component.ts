...
export class RepeaterComponent implements OnInit, OnDestroy, ControlValueAccessor {
    ...
    public writeValue(value: any) {
        ...
        this.control.clear();
        value.map((item) => {
            const cleanedItem = Object.keys(item).reduce((acc, itemEntryKey) => {
                if (Array.isArray(item[itemEntryKey])) {
                    return {
                        ...acc,
                        [itemEntryKey]: [item[itemEntryKey]]
                    };
                }
            ...
            this.control.push(this.formBuilder.group(cleanedItem));
        });
    }
